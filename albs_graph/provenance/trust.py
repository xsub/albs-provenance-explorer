...
def find_binary_rpm(graph: ProvenanceGraph, name_or_node: str, arch: str | None = None) -> Node:
    ...
    if len(matches) > 1:
        ...
        raise ValueError(f"Ambiguous RPM {name_or_node}; use --arch or a full node id. Matches: {choices}{more}")
    return matches[0]
...
def focused_trust_graph(
    ...
    if build_task_id:
        selected.update(edge.source for edge in graph.incoming(build_task_id, Relation.DERIVED_FROM))
        ...
        if include_tests:
            selected.update(edge.target for edge in graph.outgoing(build_task_id, Relation.TESTED_BY))
