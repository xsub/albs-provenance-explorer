...
The suite has **500 tests** across `tests/`. They are **fully offline** — no test
...
figure is the **500** total (`scripts/check-test-count.sh`).
...
| `test_git_source.py` | 17 | On-demand git-commit inspection (D144): Gitea remote parsing (https / scp-like / scheme-less, rejecting the `unknown-albs-source` placeholder), the commit-API / web / `.diff` URL builders, the `GetSingleCommit` JSON parser, `fetch_git_commit` (injected fetcher, web-URL fill on error, degraded for an unusable remote), and `split_unified_diff` / `file_diff_from` per-file slicing with basename fallback. |
