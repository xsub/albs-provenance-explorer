...
- **Keep `ChangeLog.txt` in sync on every commit.** It is the chronological log of *how* things shipped: commit short-hash + one-bullet summary + reference to the matching `docs/decisions.md` D-entry / F-finding. `decisions.md` captures the *what* and *why*; `ChangeLog.txt` captures the *how it landed*. Newest at the top.
- **No AI attribution in commit messages.** Never add a `Co-Authored-By: Claude` trailer (or any "Generated with Claude / Claude Code" line) to commits. A local `.git/hooks/commit-msg` hook strips these as a safety net, but do not rely on it - omit them in the first place.
